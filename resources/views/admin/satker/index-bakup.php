            ...
            <div
                ...
                <form action="{{ route('admin.satker.store') }}" method="POST" class="p-6 space-y-5 bg-white">
                    ...
                    <div id="container_status_jabatan" class="hidden space-y-5">
                        <div>
                            <label class="block text-xs font-bold text-slate-700 uppercase mb-2">Jabatan</label>
                            <select name="tanpa_jabatan" id="tanpa_jabatan" onchange="handleJabatanChange()"
                                class="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition">
                                <option value="">-- Pilih Status Jabatan --</option>
                                <option value="ada">Ada Jabatan Fungsional</option>
                                <option value="tidak_ada">Tanpa Jabatan</option>
                            </select>
                        </div>
                    ...
                    <div id="container_kategori_unit" class="hidden">
                        <label class="block text-xs font-bold text-slate-700 uppercase mb-2">Kategori Unit</label>
                        <select name="kategori_unit" id="kategori_unit" onchange="handleKategoriUnitChange()"
                            ...
                            <option value="non_tu">Non Tata Usaha</option>
                        </select>
                    </div>
                    ...
                    <div id="container_kategori_kotakab" class="hidden space-y-5">
                        <div>
                            ...
                            <select id="kategori_kotakab" onchange="handleKategoriKotaKabChange()"
                                ...
                                <option value="tu_kab">Tata Usaha</option>
                                <option value="madrasah">Madrasah</option>
                                <option value="bimas_islam">Bimas Islam</option>
                            </select>
                        </div>
                        ...
                        <div id="container_jenis_madrasah" class="hidden">
                            ...
                            <select id="jenis_madrasah" onchange="handleJenisMadrasahChange()"
                                ...
                                <option value="MIN">Madrasah Ibtidaiyah Negri</option>
                                <option value="MTsN">Madrasah Tsanawiyah Negri</option>
                                <option value="MAN">Madrasah Aliyah Negri</option>
                            </select>
                        </div>
                    </div>
...
@push('scripts')
    ...
    <script>
        ...
        function handleKategoriKotaKabChange() {
            resetKodeSatker();
            const kategori = document.getElementById('kategori_kotakab').value;
            ...
            if (kategori === 'madrasah') {
                madrasahDiv.classList.remove('hidden');
                namaSatkerInput.value = "Madrasah";
            } else {
                madrasahDiv.classList.add('hidden');
                document.getElementById('jenis_madrasah').value = "";
                if (kategori !== "") {
        ...
        function handleJenisMadrasahChange() {
            ...
            if (jenisMadrasah.value !== "") {
                // Ambil teks dari dropdown (misal: "Madrasah Aliyah Negeri")
                const staticName = jenisMadrasah.options[jenisMadrasah.selectedIndex].text;

                // Simpan teks statis di atribut data untuk divalidasi nanti
        ...
        async function generateSatkerCode(event) {
            ...
            const jabatanElement = document.getElementById('jabatan_id');

            let jabatanId = "";
            let namaJabatan = "";
            let kodeFungsionalRaw = "";

            // ambil data jabatan
            if (jabatanElement.tomselect) {
                jabatanId = jabatanElement.tomselect.getValue();
                const selectedData = jabatanElement.tomselect.options[jabatanId];
                if (selectedData) {
                    namaJabatan = selectedData.text.toLowerCase();
                    kodeFungsionalRaw = selectedData.text.split(' - ')[0].trim();
                }
            } else {
                jabatanId = jabatanElement.value;
                if (jabatanId) {
                    const selectedOption = jabatanElement.options[jabatanElement.selectedIndex];
                    namaJabatan = selectedOption.text.toLowerCase();
                    kodeFungsionalRaw = selectedOption.text.split(' - ')[0].trim();
                }
            }
            ...
            try {
                ...
                // =================================
                // PENENTUAN KODE TENGAH
                // =================================

                const kategoriUnit = document.getElementById('kategori_unit').value; // Untuk Kanwil
                const kategoriKotaKab = document.getElementById('kategori_kotakab').value; // Untuk Kota/Kab

                if (statusJabatan === 'tidak_ada') {
                    middle = "00";
                } else if (statusJabatan === 'pelaksana') {
                    middle = "7";
                } else if (statusJabatan === 'jabatan_kanwil') {
                    // Logika Kanwil: TU = 01, Lainnya = Slice dari server
                    if (kategoriUnit === 'tu') {
                        middle = "01";
                    } else {
                        const prefixLength = parentKode.length;
                        middle = fullCode.slice(prefixLength, prefixLength + 2);
                    }
                } else if (statusJabatan === 'jabatan_kotakab') {
                    // Logika Baru Kota/Kab berdasarkan kategori yang dipilih
                    if (kategoriKotaKab === 'tu_kab') {
                        middle = "01";
                    } else if (kategoriKotaKab === 'madrasah') {
                        middle = "02";
                    } else if (kategoriKotaKab === 'bimas_islam') {
                        middle = "03";
                    } else {
                        // Fallback jika belum pilih kategori tapi menekan generate
                        middle = "21";
                    }
                } else if (statusJabatan === 'manajerial') {
                    const prefixLength = parentKode.length;
                    middle = fullCode.slice(prefixLength, prefixLength + 2);
                } else if (statusJabatan === 'ada') {
                    middle = kodeFungsionalRaw;
                } else {
                    // Mapping jabatan lainnya (wakil_rektor, dekan, dll)
                    const mapping = {
                        wakil_rektor: "91",
                        dekan: "901",
                        wakil_dekan: "9011",
                        ketua_jurusan: "90101",
                        wakil_jurusan: "901011",
                        ketua_prodi: "9010101",
                        seketaris_prodi: "90101011",
                        lembaga: "921",
                        sekretaris_lembaga: "9211",
                        pusat: "92101",
                        sekretaris_kepala_pusat: "921011",
                        jabatan_kotakab: "21"
                    };
                    middle = mapping[statusJabatan] || "";
                }
                ...
                if (jenisId === "1") {
                    // LOGIKA ESELON 1: Kode otomatis dari BE, tapi field dibuat editable (flex-1)
                    // Nama Satker dibiarkan manual (free text)
                    container.insertAdjacentHTML('afterbegin', `
        <input type="text" name="kode_parts[]" value="${middle || fullCode}" required
        class="flex-1 px-4 py-2 bg-white border border-blue-300 rounded-xl text-sm focus:ring-2 focus:ring-blue-400 outline-none"
        placeholder="Masukkan Kode Eselon 1...">
    `);

                    // Opsional: Kosongkan nama satker agar user bisa mengisi free text jika belum diisi
                    const namaInput = document.getElementById('nama_satker');
                    if (namaInput.value === 'Tidak Ada Jabatan') namaInput.value = '';

                } else {
                    // LOGIKA ESELON 2 KEATAS (Sama seperti kode lama Anda)
                    let htmlInputs = `
        <input type="text" value="${prefix}" readonly
        class="w-20 px-3 py-2 bg-slate-200 border rounded-xl text-sm text-center">

        <input type="text" value="${middle}" readonly
        class="w-24 px-3 py-2 bg-slate-200 border rounded-xl text-sm text-center">
    `;

                    if (suffix && statusJabatan !== 'tidak_ada' && statusJabatan !== 'pelaksana') {
                        htmlInputs += `
        ...
        `;
                ...
                // Selalu panggil ini untuk menggabungkan kode ke hidden input 'kode_satker_full'
                updateFullCode();
    ...
    <script>
        ...
        function updateNamaSatkerDariKabupaten() {
            ...
            // Pastikan fungsi ini hanya berjalan jika status jabatan adalah jabatan_kotakab
            if (statusJabatan === 'jabatan_kotakab' && kabupatenSelect.value !== "") {
                const selectedText = kabupatenSelect.options[kabupatenSelect.selectedIndex].text;

                // Mengisi nama satker dengan nama kabupaten (Contoh: KAB. BANYUWANGI)
                namaSatkerInput.value = selectedText;
            }
        }
        ...
        function handleJabatanChange() {
            ...
            const fungsionalDiv = document.getElementById('container_jabatan_fungsional');
            const kabupatenDiv = document.getElementById('container_kabupaten');
            const kategoriUnitDiv = document.getElementById('container_kategori_unit');
            const kategoriKotaKabDiv = document.getElementById('container_kategori_kotakab');
            const namaSatkerInput = document.getElementById('nama_satker');

            // 1. Reset awal (sembunyikan semua)
            kategoriUnitDiv.classList.add('hidden');
            kategoriKotaKabDiv.classList.add('hidden');
            kabupatenDiv.classList.add('hidden');
            fungsionalDiv.classList.add('hidden');
            document.getElementById('kategori_unit').removeAttribute('required');
            document.getElementById('kabupaten_id').removeAttribute('required');

            // --- TAMBAHAN KHUSUS ESELON 1 ---
            // Jika Eselon 1, hentikan logika otomatisasi nama agar tetap free text
            if (jenisId === "1") {
                // Kosongkan nama jika sebelumnya berisi 'Tidak Ada Jabatan' 
                // agar user bisa mengisi manual dari awal
                if (namaSatkerInput.value === 'Tidak Ada Jabatan') {
                    namaSatkerInput.value = '';
                }
                return;
            }
            ...
            // 2. Logika Kanwil (Khusus Eselon 3)
            if (jenisId === "3" && status === 'jabatan_kanwil') {
                kategoriUnitDiv.classList.remove('hidden');
                document.getElementById('kategori_unit').setAttribute('required', 'required');
            }

            // 3. Logika Kota/Kab (Muncul kapanpun status 'jabatan_kotakab' dipilih)
            if (status === 'jabatan_kotakab') {
                kabupatenDiv.classList.remove('hidden');
                document.getElementById('kabupaten_id').setAttribute('required', 'required');

                if (jenisId === "4") {
                    kategoriKotaKabDiv.classList.remove('hidden');
                }

                updateNamaSatkerDariKabupaten();
            }

            // 4. Logika Nama Satker Otomatis
            if (status === 'tidak_ada') {
                namaSatkerInput.value = 'Tidak Ada Jabatan';
            } else if (status === 'ada') {
                fungsionalDiv.classList.remove('hidden');
                namaSatkerInput.value = '';
            } else if (status === 'jabatan_kotakab') {
                // Nama dihandle updateNamaSatkerDariKabupaten
            } else if (status !== "") {
                const sel = document.getElementById('tanpa_jabatan');
                namaSatkerInput.value = sel.options[sel.selectedIndex].text;
            }
        }
        ...
        function handleWilayahChange() {
            ...
            const selectedText = wilayahSelect.options[wilayahSelect.selectedIndex].text.toUpperCase();

            // Simpan nilai lama untuk pengecekan (opsional)
            ...
            if (selectedText.includes('PUSAT')) {
                options = [{
                        val: 'manajerial',
                        label: 'Manajerial'
                    },
                    ...
                    {
                        val: 'tidak_ada',
                        label: 'Tidak Ada Jabatan'
                    }
                ];
            } else if (selectedText.includes('PTKN') || selectedText.includes('PERGURUAN TINGGI')) {
                options = [{
                        val: 'manajerial',
                        label: 'Manajerial'
                    },
                    ...
                    {
                        val: 'sekretaris_kepala_pusat',
                        label: 'Sekretaris Kepala Pusat'
                    },
                    {
                        val: 'tidak_ada',
                        label: 'Tidak Ada Jabatan'
                ...
                // Default (Kanwil/Lainnya)
                options = [{
                        val: 'jabatan_kanwil',
                        label: 'Jabatan di Kanwil'
                    },
                    {
                        val: 'jabatan_kotakab',
                        label: 'Jabatan di Kota/Kab'
                    },
                    {
                        val: 'ada',
                        label: 'Jabatan Fungsional'
            ...
            // Reset kontainer fungsional jika wilayah berubah
            handleJabatanChange();
        }
    ...
    `;
            ...
            try {
                ...
                if (users.length > 0) {
                    ...
                    users.forEach((user, index) => {
                        ...
                        tableBody.innerHTML += `
                        <tr class="hover:bg-blue-50/50 transition duration-200" data-name="${user.name.toLowerCase()}" data-nip="${user.nip ?? ''}">
        ...
        document.getElementById('searchUserDetail').addEventListener('keyup', function() {
            ...
            rows.forEach(row => {
                // jika baris skeleton / pesan tidak ada data, tampilkan terus
                if (!row.dataset.name) {
                    row.style.display = '';
                    return;
                }
                ...
                let nip = (row.dataset.nip ?? '').toLowerCase();

                if (keyword === '' || name.includes(keyword) || nip.includes(keyword)) {
                    row.style.display = '';
